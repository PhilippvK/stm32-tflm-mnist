...
uint8_t* get_data(const char* path, char* file, FSIZE_t size) {
  ...
  if (FATFS_LinkDriver(&SD_Driver, sdpath) != 0) {
    ...
    if (f_mount(&sdfs, (TCHAR const*)sdpath, 0) != FR_OK) {
      ...
      sprintf(buf, "%s/%s", path, file);  // TODO(PhilippvK): snprintf
          ...
          if (index < MAX_FILES) {
            if (fno.fname[0] != '.') {
              ...
              if (!files) {
                ...
                sprintf(files[index++], "%s",
                        fno.fname);  // TODO(Philippvk): snprintf
              }
            }
          }
...
uint32_t test_txt_file() {
  ...
  if (FATFS_LinkDriver(&SD_Driver, sdpath) == 0) {
    ...
    if (f_mount(&sdfs, (TCHAR const*)sdpath, 0) != FR_OK) {
      ...
      /* WARNING: Formatting the uSD card will delete all
       *          content on the device */
      // if(f_mkfs((TCHAR const*)sdpath, FM_ANY, 0,
      //           buffer, sizeof(buffer)) != FR_OK)
      if (0) {  // SKIPPED
        /* FatFs Format Error */
        FATFS_UnLinkDriver(sdpath);
        return 0;
