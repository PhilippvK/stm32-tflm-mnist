...
void SendImageUART(void *image, size_t pixel_size, size_t image_size,
                          char flags);
void DrawShrunkImage(int x, int y);
...
void DrawShrunkImage(int x, int y) {
  for (uint32_t i = 0; i < INPUT_IMAGE_SIZE; i++) {
    for (uint32_t j = 0; j < INPUT_IMAGE_SIZE; j++) {
...
void SaveMNISTInput(void) {
  ...
  PreprocessImage();
  // TODO(PhilippvK): clear Touch input
#else
  volatile int marginX = INPUT_BOX_X_INNER +
...
void DrawInputScreen(void) {
  ...
  if (prev_value != -1) {
    char buf[2];
    sprintf(buf, "%c", prev_value);  // TODO(PhilippvK): snprintf
    BSP_LCD_DisplayStringAt(0, 200, (uint8_t *)buf, CENTER_MODE);
  } else {
  // BSP_LCD_DisplayStringAt(0, 200,
  //     (uint8_t *)"Press USER for classification", CENTER_MODE);
  }
}
...
void PrintMNISTHint() {
  ...
  BSP_LCD_DisplayStringAt(0, INPUT_BOX_Y_INNER+INPUT_IMAGE_SIZE_PIXEL-60,
                          (uint8_t *)"Start Drawing and", CENTER_MODE);
  BSP_LCD_DisplayStringAt(0, INPUT_BOX_Y_INNER+INPUT_IMAGE_SIZE_PIXEL-40,
                          (uint8_t *)"press USER when done", CENTER_MODE);
}
  ...
  if (CheckForUserInput()) {
    ...
    if (DrawInBox(x, y, dot_radius)) {
      if (x0 >= 0 && y0 >=0) {
        ...
        if ((uint32_t)(abs(x-x0)+abs(y-y0)) <= dot_radius*2) {
