  ...
  while (index < TYPE_COUNT) {
    if (mask & types) {
      fprintf(stderr, "Ticks for %s (cur/min/max/avg): %ld/%ld/%ld/%ld\n\r",
              tick_labels[index], ticks_infos[index].cur,
              ticks_infos[index].min, ticks_infos[index].max,
              ticks_infos[index].avg);
    }
    index++;
    mask <<= 1;
  }
...
void update_avg_ticks(int type, int32_t ticks) {
  ...
  if (ticks_infos[index].count < AVG_LEN) {
    ...
    ticks_infos[index].sum -=
      ticks_infos[index].ticks[ticks_infos[index].index];
  }
