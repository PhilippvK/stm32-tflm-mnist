...
#define AUDIO_WAV_HEADER_OFFSET 44  /* Offset relative to file header size */
...
void AudioPlay(uint8_t* data, uint32_t size) {
  ...
  (BSP_AUDIO_OUT_TransferComplete_CallBack() or
  BSP_AUDIO_OUT_HalfTransfer_CallBack()...
  */
  AUDIO_Start(wav_addr+AUDIO_WAV_HEADER_OFFSET,
              wav_size-AUDIO_WAV_HEADER_OFFSET);
}
...
void AudioInit(void) {
  ...
  if (BSP_AUDIO_OUT_Init(OUTPUT_DEVICE_HEADPHONE, uwVolume,
                         *AudioFreq_ptr) == 0) {
    // TODO(PhilippvK): Error handling
  }
}
