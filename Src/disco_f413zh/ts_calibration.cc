...
#define CALIB_RES(val, pte1, ptm1, pte2, ptm2) \
  ((((val-ptm2)*(pte1-pte2))/(ptm1-ptm2))+pte2)
...
static void TouchscreenCalibration_SetHint(void) {
  ...
  BSP_LCD_DisplayStringAt(0, BSP_LCD_GetYSize() / 2 - 35,
                          (uint8_t *)"Before using the Touchscreen",
                          CENTER_MODE);
  BSP_LCD_DisplayStringAt(0, BSP_LCD_GetYSize() / 2 - 15,
                          (uint8_t *)"you need to calibrate it.",
                          CENTER_MODE);
  BSP_LCD_DisplayStringAt(0, BSP_LCD_GetYSize() / 2 + 7,
                          (uint8_t *)"Wait until black circles appear,",
                          CENTER_MODE);
  BSP_LCD_DisplayStringAt(0, BSP_LCD_GetYSize() / 2 + 30,
                          (uint8_t *)"Then press precisly on them.",
                          CENTER_MODE);
  BSP_LCD_SetFont(&Font12);
}
...
static void TouchScreen_Calibration_GetPhysValues(int16_t LogX, int16_t LogY,
  ...
  /* Return as physical touch values the positions of first touch,
   * even if double touched occurred */
...
static void TouchScreen_Calibration_WaitForPressedState(uint8_t Pressed) {
  ...
  uint32_t exitFirstLevelWhileLoopReq = 0;
  // By default no exit request from first level while loop
  uint32_t exitSecondLevelWhileLoopReq = 0;
  // By default no exit request from second level while loop
  ...
  do {
    ...
    /* Sense of touch state from touch IC until get the awaited state
     * in parameter 'Pressed' */
    status = BSP_TS_GetState(&TS_State);
    if (status == TS_OK) {
      if (((Pressed == 0) && (TS_State.touchDetected == 0)) ||
          ((Pressed == 1) && ((TS_State.touchDetected == 1) ||
          (TS_State.touchDetected == 2)))) {
        ...
        do {
          /* New sense of touch state from touch IC :
           * to evaluate if state was stable */
          status = BSP_TS_GetState(&TS_State);
          if (status == TS_OK) {
            /* Is there a state change compared since having found
             * the awaited state ? */
            if (((Pressed == 0) && ((TS_State.touchDetected == 1) ||
                    (TS_State.touchDetected == 2))) ||
                ((Pressed == 1) && ((TS_State.touchDetected == 0)))) {
              /* Too rapid state change => anti-rebound management :
               * restart first touch search */
              exitSecondLevelWhileLoopReq = 1;
              /* exit request from second level while loop */
            } else if ((HAL_GetTick() - 100) > TimeStart) {
              /* State have not changed for the timeout duration
               * (stable touch for 100 ms) */
    ...
    /* Scan User button in case early exit requested by user to switch to
     * an other use case while calibrating */
    /* the touch screen                                              */
